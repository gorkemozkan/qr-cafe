...
export abstract class BaseRepository {
  ...
  protected async post<T>(endpoint: string, body?: Record<string, unknown>, options?: RequestInit): Promise<ApiResponse<T>> {
    return apiClient.post<T>(`${this.baseUrl}${endpoint}`, body, options);
  }

  protected async put<T>(endpoint: string, body?: Record<string, unknown>, options?: RequestInit): Promise<ApiResponse<T>> {
  ...
  protected async patch<T>(endpoint: string, body?: Record<string, unknown>, options?: RequestInit): Promise<ApiResponse<T>> {
