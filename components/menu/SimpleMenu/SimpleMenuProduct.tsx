  ...
  return (
    ...
    >
      <div className="flex-1">
        <div
          className={`flex items-center ${!product.is_available ? "text-muted-foreground line-through" : "text-[#8B1538] dark:text-[#A61E4D]"}`}
        >
          <div className="flex justify-between items-center w-full">
            ...
            {!!product.price && product.price > 0 && currency && (
              <p className="flex-shrink-0 block">{formatPrice(product.price, currency)}</p>
            )}
          </div>
        </div>
        {product.description && (
          <p className="text-gray-500 dark:text-gray-400 text-xs mt-1 leading-relaxed font-normal italic">
            {product.description}
          </p>
        )}
      </div>
